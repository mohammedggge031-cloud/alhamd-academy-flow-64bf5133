...
const Dashboard = () => {
  ...
  // Recent bookings & subscriptions
  const { data: recentBookings = [] } = useQuery({
    queryKey: ["dash-recent-bookings"],
    queryFn: async () => {
      const { data } = await supabase
        .from("trial_bookings")
        .select("id, full_name, phone, course_interest, status, is_read, created_at")
  ...
  const { data: recentSubs = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        ...
        .select("id, full_name, phone, plan_name, status, is_read, created_at")
        .order("created_at", { ascending: false })
        .limit(5);
      return data ?? [];
    },
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <Card className="border-none shadow-sm">
          ...
          <CardContent className="space-y-2">
            {recentBookings.length === 0 ? (
              <p className="text-center text-sm text-muted-foreground py-3">لا توجد حجوزات</p>
              ...
              <div key={b.id} className={`flex items-center justify-between rounded-lg p-3 ${!b.is_read ? "bg-primary/5" : "bg-muted/50"}`}>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">{b.course_interest || "حصة تجريبية"}</p>
                  </div>
                </div>
        ...
        <Card className="border-none shadow-sm">
          ...
          <CardContent className="space-y-2">
            {recentSubs.length === 0 ? (
              <p className="text-center text-sm text-muted-foreground py-3">لا توجد طلبات اشتراك</p>
